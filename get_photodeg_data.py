...
def get_photodeg_from_mongodb(content):
    ...
    df["photodeg_kinetic_rate"] = [
        data_entries[_id]["kinetic_rate"] for _id in ids
    ]
    df["mongodbid"] = ids

    df.dropna(subset=["smiles", "photodeg_kinetic_rate"], inplace=True)
...
def main():
    content = run_mongodb_query(
        search_type="special_search", collection="data", search_term=["exp_kinetic_abs"]
    )
    df = get_photodeg_from_mongodb(
        content
    )
    ...
    create_new_model_directory(
        new_model_dir_name="photodeg_model_v4",
        new_results_dir_name="v3",
        new_results_file_name="v3.csv",
        property="photodeg",
    )
